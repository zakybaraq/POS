export function getSidebarHtml(activePage: string, user: { role: string; name: string }) {
  const roleMenuMap: Record<string, string[]> = {
    super_admin: ['dashboard', 'admin', 'pos', 'menu', 'tables', 'orders', 'inventory', 'customers', 'reports', 'settings'],
    admin_restoran: ['pos', 'menu', 'tables', 'orders', 'inventory', 'customers', 'reports', 'settings'],
    kasir: ['pos', 'customers'],
