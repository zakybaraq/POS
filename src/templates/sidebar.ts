export function getSidebarHtml(activePage: string, user: { role: string; name: string }) {
  const roleMenuMap: Record<string, string[]> = {
    super_admin: ['dashboard', 'admin', 'pos', 'menu', 'tables', 'orders', 'inventory', 'customers', 'reports', 'settings', 'suppliers', 'purchase-orders'],
    admin_restoran: ['pos', 'menu', 'tables', 'orders', 'inventory', 'customers', 'reports', 'settings', 'suppliers', 'purchase-orders'],
    kasir: ['pos', 'customers'],
